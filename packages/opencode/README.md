...
`@forge/opencode` runs a Forge actor through the local [opencode](https://opencode.ai) CLI in non-interactive mode (`opencode run`), giving Forge factories access to every provider opencode is authenticated with — including its free model tier.
