...
```sh
bun run deliver "Write a haiku file" --factory local --backlog fixtures/haiku.json --budget 5
```
...
```sh
bun run deliver "Create a tiny static site: an index.html, a styles.css, and an about page, linked together" --factory local --budget 15
```
...
```sh
bun run deliver "Write a config file for the deployment" --factory local --budget 8
```
...
```sh
bun run deliver "Deliver a robust CSV parser" --factory local --backlog fixtures/csv-parser.json --budget 10
```
...
```sh
bun run deliver "Create three markdown files: todo.md, done.md, ideas.md, each with a heading" --factory local --budget 3
```
...
bun run deliver "<same task>" --factory local --decisions /tmp/forge-voyage/answers.json --budget 8
```
