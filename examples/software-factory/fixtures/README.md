...
```sh
bun run forge run "Write a haiku file" --assembly local --backlog fixtures/haiku.json --budget 5
```
...
```sh
bun run forge run "Create a tiny static site: an index.html, a styles.css, and an about page, linked together" --assembly local --budget 15
```
...
```sh
bun run forge run "Write a config file for the deployment" --assembly local --budget 8
```
...
```sh
bun run forge run "Deliver a robust CSV parser" --assembly local --backlog fixtures/csv-parser.json --budget 10
```
...
```sh
bun run forge run "Create three markdown files: todo.md, done.md, ideas.md, each with a heading" --assembly local --budget 3
```
...
```sh
echo '[{ "question": "<from the previous result decisions log>", "answer": "<your answer>" }]' > /tmp/forge-voyage/answers.json
bun run forge run "<same task>" --assembly local --decisions /tmp/forge-voyage/answers.json --budget 8
```
